...
Mat3::Mat3(std::initializer_list<float> out_initList)
{
	if (out_initList.size() < 9)
		std::cout << "Insufficient elements" << std::endl;
	else
	{
		auto it = out_initList.begin();
		for (int i = 0; i < 3; ++i)
			for (int j = 0; j < 3; ++j)
...
Mat3::Mat3(const Mat3& out_other)
{
	for (int i = 0; i < 3; ++i)
		for (int j = 0; j < 3; ++j)
			this->m_matrix[i][j] = out_other.m_matrix[i][j];
}
